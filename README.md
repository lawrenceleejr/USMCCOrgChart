...
SVGs, separately) as workflow artifacts — these live at the bottom of an
individual run's summary page, download as a zip, and expire after 90 days.

Pushes to `main` also publish to GitHub Pages: both variants shown over the
background each is meant for, with every format one click away. This needs
**Settings → Pages → Source: GitHub Actions** switched on once.
...
| `scripts/detect-faces.py` | Measures face positions in a reference image |
| `scripts/build-site.py` | Builds the GitHub Pages site into `site/` |
...
`config/source-layout.yaml` records where the faces sit *in that source
image*, and is generated by measurement rather than by eye:

```
scripts/detect-faces.py reference/usmcc-leadership-nov2025.png
```

which finds the headshot rings as dark connected components and matches
each to the nearest known id. Those coordinates are deliberately
